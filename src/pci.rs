...
impl DeviceConfig {
    ...
    pub fn disable_bus_mastering(&mut self) {
        let mut register = ConfigRegister::new(self.bus, self.device, self.function, 0x04);
        let mut data = register.read();
        data.set_bit(2, false);
        register.write(data);
    }

    pub fn io_base(&mut self) -> u16 {
        self.enable_bus_mastering();
        let io_base = (self.base_addresses[0] as u16) & 0xFFF0;
        io_base
    }
}
...
fn add_device(bus: u8, device: u8, function: u8) {
    ...
    println!("PCI {:04X}:{:02X}:{:02X} [{:04X}:{:04X}]", bus, device, function, config.vendor_id, config.device_id);
}
...
pub fn find_pci_io_base(vendor_id: u16, device_id: u16) -> Option<u16> {
    if let Some(mut pci_device) = find_device(vendor_id, device_id) {
