use crate::{arch::{self, cmos::{CMOS, self}}, klog, csh::{ShellArgs, ExitCode}, println};
...
pub fn time(_: ShellArgs) -> ExitCode {
    let rt = CMOS::new().rtc();
    println!("Time: {}:{}:{} - {}/{}/{} - Uptime: {}", rt.hour, rt.minute, rt.second, rt.day, rt.month, rt.year, seconds());
    ExitCode::Ok
}
...
pub fn seconds() -> f64 {
    unsafe {
        (RTC_TICKS as f64) / (UPDATE_FREQ as f64)
    }
}
